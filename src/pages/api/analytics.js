...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      ...
      // This is a simple linear prediction model with bounds checking
      ...
      if (weightData.length >= 2) {
        ...
        if (latestWeight?.weight && previousWeight?.weight) {
          // Get weight trend based on data
          ...
          // Cap the daily weight change to a reasonable amount 
          // Most people shouldn't lose/gain more than 2lbs (0.9kg) per week or ~0.3lbs per day
          const cappedDailyChange = Math.max(-0.3, Math.min(0.3, dailyWeightChange));
          ...
          for (let i = 1; i <= 4; i++) {
            ...
            // Calculate predicted weight with a minimum threshold
            // Ensure weight doesn't go below a reasonable minimum (e.g., 40kg/88lbs)
            const minWeight = 88; // minimum reasonable weight in pounds
            const predictedWeight = Math.max(
              minWeight, 
              latestWeight.weight + (cappedDailyChange * i * 7)
            );
